...
describe('ErrorDeduplicationService', () => {
  ...
  it('creates a DB record and caches first occurrence when key is not found', async () => {
    ...
    prisma.externalError.create.mockResolvedValue({ id: 123 });
    ...
    expect(prisma.externalError.create).toHaveBeenCalledWith({
      data: {
    ...
    expect(prisma.externalError.update).not.toHaveBeenCalled();
  ...
  it('increments count and refreshes cache when duplicate key is found', async () => {
    ...
    expect(prisma.externalError.update).toHaveBeenCalledWith({
      where: { id: 555 },
      data: { count: { increment: 1 } },
    });
    expect(prisma.externalError.create).not.toHaveBeenCalled();
  ...
  it('uses 300 seconds as default TTL when config is missing', async () => {
    ...
    redis.get.mockResolvedValue(null);
    prisma.externalError.create.mockResolvedValue({ id: 999 });
